...
describe('silly.svg', () => {
    ...
    testVNodes('vnodes have right content', './silly.svg', data, {}, vnodes => {
        expect(
            vnodes.filter(({ nodeName }) => nodeName === 'text')
        ).to.deep.equal([
            {
                nodeName: 'text',
                attributes: { class: 'notfill' },
                children: [
                    "We need's to make sure \\ that all sorts of funky § gets through? &quot;",
                ],
            },
        ]);
    });
    ...
    testVNodes(
        'passes width/height/class/etc to root vnode',
        './silly.svg',
        data,
        {
            props: {
    ...
    testVNodes(
        'does class replacement with a "styles" prop',
        './silly.svg',
        data,
        {
            props: {
        ...
        vnodes => {
            const classes = vnodes
                .map(({ attributes: { class: className } }) => className)
                .filter(Boolean);
